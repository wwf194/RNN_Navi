...
class Arena2D():
    ...
    def InitFromParam(self, param):
        ...
        for ShapeParam in param.Shapes:
            self.Shapes.append(World2D.Shapes2D.BuildShape(ShapeParam))
        self.CalculateBoundaryBox()
    def CalculateBoundaryBox(self):
        param = self.param
        BoundaryBoxes = []
        for Shape in self.Shapes:
            BoundaryBoxes.append(Shape.param.BoundaryBox)
        BoundaryBoxes = np.stack(BoundaryBoxes, axis=0) # [ShapeNum, (xMin, yMin, xMax, yMax)]
        BoundaryBox = [np.min(BoundaryBoxes[:, 0]), np.min(BoundaryBoxes[:, 1]),
            np.max(BoundaryBoxes[:, 0]), np.max(BoundaryBoxes[:, 1])]
        if HasAttrs(param, "BoundaryBox") and isinstance(GetAttrs(param.BoundaryBox), list):
            if not utils_torch.geometry.RectangleAContainsRectangleB(GetAttrs(param.BoundaryBox), BoundaryBox):
                raise Exception()
            else:
                return
        else:
    ...
    def IsInside(self, Points):
        ...
        isOutside = np.empty_like(Points, type=np.bool8)
        for Shape in self.Shapes:
            isOutside *= Shape.IsOutside(Points)
        return isOutside
    ...
    def GenerateRandomPointsInBoundaryBox(self, Num):
        param = self.param
        return np.stack([np.random.uniform(param.xMin, param.xMax, Num), np.random.uniform(self.y0, self.y1, Num)], axis=0) #[points_num, (x, y)]
    def GenerateRandomInternalPoints(self, Num=100, MinDistance2Border=0.0):
        ...
        while(PointsNum < Num):
            ...
            Points = np.delete(Points, self.isOutside(Points, MinDistance2Border=MinDistance2Border), axis=0)
